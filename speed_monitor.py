...
class SpeedMonitor:
    ...
    def __init__(self, num_cams: int = 4):
        ...
        with self._lock:
            ...
            for cam in range(self.num_cams):
                ...
                if ifps > 0.1 and pfps < ifps * 0.85:
                    warn = f"  [!] 处理速度({pfps:.2f}fps)落后输入({ifps:.2f}fps)，队列可能堆积！"
                elif qs >= 150:
                    warn = f"  [!] 队列水位={qs}（接近上限200），注意堆积风险！"
                else:
                    warn = ""

                lines.append(
                    f"  CAM{cam + 1}: input={ifps:.2f}fps  proc={pfps:.2f}fps  qsize={qs}{warn}"
                )
                lines.append(f"         阶段耗时: {stage_str}")
...
def init_monitor(num_cams: int = 4) -> SpeedMonitor:
    """初始化并启动全局速度监控器（在 run_online() 入口调用一次）。"""
    global _global_monitor
    _global_monitor = SpeedMonitor(num_cams=num_cams)

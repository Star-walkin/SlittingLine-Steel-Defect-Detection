...
def filter_classes(A, class_list):
    ...
    for item in A:
        ...
        try:
            ...
            if class_value not in class_list:
                continue
...
def gen_detect_report(test_path,fukuan0,conduct_id,anomaly_area_cls_range,remove_threshold,
                      ...
                      class_labels,area_range,strip_id,result_all_path):
    ...
    #test_folder_path = os.path.join(test_path, "20251008", conduct_id)
    anomaly_info_process_final = Anomaly_info_List(filepath=test_path)
    fukuan_list=anomaly_info_process_final.load_anomaly_info(calibrat_cam_id, "fukuan")
    #fukuan_mean = int(sum(fukuan_list)/len(fukuan_list))
    fukuan_max=max(fukuan_list)
    fukuan_min=min(fukuan_list)
    fukuan_fig(fukuan_list,fukuan0,strip_dir,strip_id,0.1575)
    ...
    Statistic = Statistic_anomaly(
        ...
        steel_length_range=steel_length_range,
    ...
    print(up_anomaly_info_all)
    print(down_anomaly_info_all)
    ...
    print(up_anomaly_info_all_for_class)
    print(down_anomaly_info_all_for_class)
...
def gen_all_reports(result_all_path, fukuan0, conduct_id, anomaly_area_cls_range, remove_threshold,
                    ...
                    camrea_id_up_list=None, strip_only_0based=None):
    """
    多带钢检测报告生成器：
    ...
    if len(strip_paths) == 0:
        ...
        gen_detect_report(
            ...
            colors, class_labels, area_range, 0, result_all_path,
        )
        print("报告生成完成。")
        return
    ...
    for strip_id, strip_path in enumerate(strip_paths):
        ...
        gen_detect_report(
            ...
            camrea_id_up_report, camrea_id_down_report, updates_up, updates_down,
            print_cls, class_list, update_info, standard_area_tables,
            colors, class_labels, area_range, strip_id, result_all_path,
        )
...
if __name__ == '__main__':
    ...
    parser.add_argument("--conduct-id", type=str, default=None, dest="conduct_id_cli", help="生产卡号")
    parser.add_argument(
        "--strip-id",
        type=int,
        default=None,
    ...
    with open(os.path.join(_PROJECT_ROOT, "config", "rptcfg.yaml"), "r", encoding="utf-8") as f:
        rptcfg = yaml.safe_load(f)
    updates_up = rptcfg["updates_up"]
    updates_down = rptcfg["updates_down"]
    print_cls = rptcfg["print_cls"]
    class_list = rptcfg["class_list"]
    update_info = rptcfg["update_info"]
    colors = rptcfg["colors"]
    class_labels = rptcfg["class_labels"]
    area_range = rptcfg["area_range"]
    ...
    if explicit_path:
        ...
        if len(folders) == 0:
            ...
            print(
                f"[ERROR] 未找到生产卡号「{conduct_id}」的检测结果目录。\n"

...
def mean_smoothing(amaps: Tensor, kernel_size1: int=9
                   ) -> Tensor:

    mean_kernel1 = torch.ones(1, 1, kernel_size1, kernel_size1) / kernel_size1 ** 2
    mean_kernel1 = mean_kernel1.to(amaps.device)
    result1=F.conv2d(amaps, mean_kernel1, padding=kernel_size1 // 2, groups=1)
...
class MSGMSLoss(Module):
    ...
    def forward(self, img1: Tensor, img2: Tensor, as_loss: bool = False) -> Tensor:
    ...
    def _create_prewitt_kernel(self) -> Tuple[Tensor, Tensor]:
        ...
        # [关键修改] 不要写死 3，改成 self.in_channels

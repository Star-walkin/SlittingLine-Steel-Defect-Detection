...
def _split_1strip_profile_percentile(gray_img, min_w):
    """
    Otsu 失败时的兜底：中间 50% 行列投影 + 分位数阈值，取最宽「亮」连续段为单条钢区。
    """
    h, w = gray_img.shape[:2]
    if h < 8 or w < 100:
        return None
    top, bot = h // 4, 3 * h // 4
    prof = np.mean(gray_img[top:bot, :], axis=0).astype(np.float32)
    k = max(15, (w // 80) | 1)
    prof = cv2.GaussianBlur(prof.reshape(1, -1), (1, k), 0).ravel()
    p_lo = float(np.percentile(prof, 10))
    p_hi = float(np.percentile(prof, 90))
    if p_hi - p_lo < 8.0:
        return None
    thr = p_lo + 0.22 * (p_hi - p_lo)
    b = prof >= thr
    ...
    for s, e in zip(starts, ends):
        width = int(e - s)
        if width >= min_w and width > best_width:
            best_width = width
            best = (int(s), int(e))
    return best


def _split_1strip_by_gradient(gray_img, fukuan_list_mm, standard_ratio_x=1.0,
                               min_strip_width_px=80, outer_safety_px=15, joint_alpha=0.5):
    """
    单条带钢：与三条带同源——中间 50% 行列均值投影 + Otsu 得亮区，取最宽连通段；
    在左/右边界附近用 1D 梯度精修外缘。

    说明：旧实现仅用 _fine_trim_strip_v2 在整幅左右各 edge_margin 像素内搜边，
    黑底很宽、钢带居中时会找不到边沿而退回 (0,W)，把整幅误当成条带。
    """
    if len(fukuan_list_mm) != 1:
        return None
    h, w = gray_img.shape[:2]
    min_w = max(int(min_strip_width_px), w // 40)

    valid_strips, roi_gray, ww = _otsu_vertical_valid_strips(gray_img, num_strips=1)
    if roi_gray is None:
        return None

    l0, r0 = None, None
    if valid_strips:
        best = max(valid_strips, key=lambda lr: lr[1] - lr[0])
        l0, r0 = int(best[0]), int(best[1])
        if r0 - l0 < min_w:
            l0, r0 = None, None

    if l0 is None:
        pr = _split_1strip_profile_percentile(gray_img, min_w)
        if pr is None:
            return None
        l0, r0 = pr

    Y = np.mean(roi_gray, axis=0).astype(np.float32)
    Y_s = cv2.GaussianBlur(Y.reshape(1, ww), (7, 1), 0).flatten()
    grad = np.gradient(Y_s)

    win = max(30, (r0 - l0) // 8, ww // 25)
    L_lo = max(0, l0 - win)
    L_hi = min(ww, l0 + win)
    L_new = _refine_outer_edge_1d(grad, L_lo, L_hi, "left", inset_px=4)
    if L_new is None:
    ...
    R_lo = max(L_new + min_w // 2, r0 - win)
    R_hi = min(ww, r0 + win)
    R_new = _refine_outer_edge_1d(grad, R_lo, R_hi, "right", inset_px=4)
    if R_new is None:
        R_new = r0
    R_new = max(L_new + 2, min(R_new, ww))

    L_new = max(0, L_new - outer_safety_px)
    R_new = min(ww, R_new + outer_safety_px)

    if R_new - L_new < min_w:
        return None
    return [(L_new, R_new)]
...
def split_multi_strips(img, fukuan_list_mm=None, standard_ratio_x=1,
    ...
    1 条：中间行 Otsu 列投影 + 最宽亮带 + 梯度外缘精修（与三带钢同源）；失败时再整幅自适应 fine_trim。
    ...
    if num_strips == 1:
        ...
        if morph_closing:
            ...
            gray_u8 = cv2.morphologyEx(gray_u8, cv2.MORPH_CLOSE, _k)
        full_W = img.shape[1]
        min_w_px = max(50, full_W // 40)
        splits_1d = _split_1strip_by_gradient(
            gray_u8,

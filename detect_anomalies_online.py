...
def get_strip_count_and_fukuan(config0):
    ...
    strip_count = min(4, max(2, raw_count))
    ...
    for task in batch:
        try:
            ...
            if ttype == "save_anomaly":
                ...
                fpath = task["fpath"]
                val = task["value"]
                # 原子 append：读 -> append -> 写 tmp -> os.replace
                try:
...
def _resolve_model_paths_in_config(cfg: dict) -> None:
    if not cfg:
        return
    for k, v in list(cfg.items()):
        if not isinstance(v, str) or not v.strip():
            continue
        if not (k.endswith("_ckpt") or "model_name" in k or k == "cls_model_path"):
            continue
        p = v.strip()
        if os.path.isabs(p):
            continue
        cfg[k] = os.path.normpath(os.path.join(_REPO_ROOT, p.replace("/", os.sep)))
...
def init_detect(conduct_id, result_path_all, Consecutive_thres_num, cam_index):
    ...
    _resolve_model_paths_in_config(config)
    ...
    try:
        while not shutdown_event.is_set():
            ...
            with (lock if hasattr(lock, "__enter__") else dummy_context()):
                ...
                # split_vis: 仅在 speed_monitor.DEBUG_IO=True 时写盘（4096×4096 大图，I/O 密集）
                if speed_monitor.DEBUG_IO:
                    _t_dbg = time.perf_counter()
                    try:
                        vis_dir = os.path.join(save_root, "split_vis")
                        os.makedirs(vis_dir, exist_ok=True)
                        H_vis, W_vis = gray.shape[0], gray.shape[1]
                        ...
                        for si, (L, R) in enumerate(last_splits):
                            ...
                            cv2.rectangle(vis, (int(L), 0), (int(R), H_vis - 1), color, 3)
                            for j in range(1, cut_ratio):
                                y = int(H_vis * j / cut_ratio)
                                cv2.line(vis, (int(L), y), (int(R), y), color, 1)
                            cv2.putText(vis, f"strip{si+1}", (int(L) + 10, 40),
...
def run_online(cwd_base_result=None):
    ...
    # ---- 启动速度监控器 ----
    _speed_mon = init_monitor(num_cams=num_cams)
    print(f"[SpeedMonitor] 速度监控已启动，每 {_speed_mon.REPORT_INTERVAL:.0f}s 汇报一次"
